...
func LeastSquares(x []float64, y []float64) (a float64, b float64) {
	...
	if len(x) != len(y) {
		log.Fatal("When the least two multiplies, the length of the two arrays is inconsistent!")
	} else {
...
func main() {
	// InitPos = []int{16, 1, 2, 0, 4, 2, 7, 1, 2, 14}
	// var maxX int
	// for _, h := range InitPos {
	// 	if h > maxX {
	// 		maxX = h
	// 	}
	// }
	...
	// var xCompact, yCompact []float64
	// var sum int
	// median1 := -1
	// for i := 0; i <= maxX; i++ {
	// 	if y[i] > 0 {
	// 		xCompact = append(xCompact, float64(i))
	// 		yCompact = append(yCompact, float64(y[i]))
	// 		sum += y[i]
	// 		if median1 < 0 && sum*2 >= len(InitPos) {
	// 			median1 = i
	// 			log.Printf("median: %d", median1)
	// 		}
	// 	}
	// }
	// a, b := LeastSquares(xCompact, yCompact)

	minFuel1 := int(1e8)
	minFuel2 := int(1e8)
	var minVer1, minVer2 int
	for ver := 1; ver < 1000; ver++ {
		var fuel1, fuel2 int
		for _, h := range InitPos {
			d := int(math.Abs(float64(ver - h)))
			fuel1 += d
			fuel2 += (d * (d + 1) / 2)
		}
		if fuel1 < minFuel1 {
	...
	log.Printf("hor1: %d, fuel1: %d, hor2: %d, fuel2: %d", minVer1, minFuel1, minVer2, minFuel2)
}
